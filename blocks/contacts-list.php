	    ...
	    <div class="grid-container">
					        ...
					        <div class="cell">
						        ...
						        <div class="contactPersonBox grid-x">
							        ...
							        <div class="cell medium-5 imgBox">
								       <a href="#"  data-open="contactModal_<?php echo($contact->ID) ?>"><?php if( !empty( $contact_image ) ): ?><img src="<?php echo esc_url($contact_image['url']); ?>" alt="<?php echo esc_attr($contact_image['alt']); ?> test" /><?php else: ?><img src="http://localhost:8888/bergenglobal.no/wordpress/wp-content/uploads/missing-image-person.png" alt="Image missing <?php echo esc_html( $contact_name ); ?>" /><?php endif; ?></a> 
							        </div>
			...
			?>
			    ...
			    <?php foreach( $contacts_list as $contact ): 
					...
					$cell_phone = get_field( 'contact_cell_phone', $contact->ID );
					$contact_email = get_field( 'contact_email', $contact->ID );
					$contact_profile_link = get_field( 'contact_profile_link', $contact->ID );
					?>
					...
					<div class="reveal large contactModal" id="contactModal_<?php echo($contact->ID) ?>" data-reveal>
						...
						<?php if( !empty( $contact_profile_link ) ): ?>
							<a href="<?php echo($contact_profile_link); ?>" class="button" target="_blank">View profile</a>
						<?php endif; ?>
					</div>
